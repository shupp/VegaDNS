...
if(!isset($_REQUEST['domain_mode']) || $_REQUEST['domain_mode'] == 'delete_cancelled') {
    ...
    if(isset($_REQUEST['domain_mode']) && $_REQUEST['domain_mode'] == 'delete_cancelled') {
        // Set delete_cacelled message
        set_msg("Delete Cancelled");
    }

    // Get group if it exists
    // if(isset($_SESSION['group'])) {
        // if($my->isMyGroup($_SESSION['group'])) {
            // $group = $_SESSION['group'];
        // } else {
            // set_msg_err("Error: you do not have privileges to access this group's domains.");
    ...
    // }
    $group = $_SESSION['group'];
    $group_name_array = $my->returnGroup($group,NULL);
    $smarty->assign('group_name', $group_name_array['name']);
    $smarty->assign('group', $group);

    if($my->canCreateDomains()) {
        $smarty->assign('new_domain_url', $base_url."&mode=domains&domain_mode=add");
    }

    if($my->canEditDomains()) {
        $smarty->assign('edit_domain_url_base', $base_url."&mode=domains&domain_mode=edit_domain");
    }
    ...
    // $q = "select * from domains where ($groupquery) $searchstring $sq ";
    ...
    $q = "select a.*, b.group_id, b.name from domains a left join groups b on a.group_id = b.group_id where ($groupquery) $searchstring $sq ";

    // sort
    $sortway = get_request_sortway();
    $sortfield = get_sortfield('domains');
    ...
    // echo "<pre>\n";print_r($out_array);echo "</pre>";exit;
    ...
    // Check permissions
    if(!$my->canCreateDomains()) {
        set_msg_err("Error: you do not have enough privileges to create domains.");
        header("Location: $base_url&mode=domains");
        exit;
    }
    // New Domain Menu
    ...
    $q = "select * from domains where domain=".$db->Quote($domain);
    ...
    $q = "insert into domains (domain,group_id,status)
        values(".$db->Quote($domain).",
        '$group_id',
        '$domain_status')";
    $result = $db->Execute($q) or die($db->ErrorMsg()."<p>query was: $q");
    ...
    if($user_info['account_type'] == 'user') {
        $q = "select * from default_records where default_type='group' and group_id='".$user_info['group_id']."'";
        $result = $db->Execute($q) or die($db->ErrorMsg());
        if($result->RecordCount() == 0) {
        ...
        $q = "select * from default_records where default_type='group' and group_id='".$user_info['user_id']."'";
        $result = $db->Execute($q) or die($db->ErrorMsg());
        if($result->RecordCount() == 0) {
            // Get system default records
            $q = "select * from default_records where default_type='system'";
    ...
    mail($supportemail,"New Inactive Domain Created",$body
, "Return-path: $supportemail\r\nFrom: \"$supportname\" <$supportemail>");
    ...
    $db->Execute($q) or die($db->ErrorMsg());
    set_msg("Domain ".$_REQUEST['domain']." deleted successfully");
    header("Location: $base_url&mode=domains");
    exit;
    ...
    while(list($key,$line) = each($domains_array)) {
        ...
        while(list($line_key,$value) = each($line)) {
            if($line_key != 'domain' && !ereg("^#", $value)) {
                ...
                if(is_array($result)) {
		            if (isset($_REQUEST['default_soa']) && ($result['type']=='S')) {
    ...
    if($user_info['account_type'] == 'user') {
        ...
        // Get owner info
        $q = "select * from accounts where user_id='".$row['owner_id']."' limit 1";
        $result = $db->Execute($q) or die($db->ErrorMsg());
        if($result->RecordCount() > 0) {
            $owner_row = $result->FetchRow();
        } else {
            $owner_row = NULL;
        }

        // Get group owner info
        $q = "select * from accounts where user_id='".$row['group_id']."' limit 1";
        $result = $db->Execute($q) or die($db->ErrorMsg());
        if($result->RecordCount() > 0) {
            $group_owner_row = $result->FetchRow();
        } else {
        ...
        if($user_info['account_type'] == 'group_admin' 
            && $user_info['user_id'] != $row['owner_id']) {

            if($row['group_id'] != $user_info['user_id']) {
        ...
        if($_REQUEST['domain_mode'] == 'change_owner') {
            ...
            $owner_id = get_user_id(strtolower($_REQUEST['email_address']));
            $q = "select 'email' from accounts where user_id='$owner_id'";
            $result = $db->Execute($q) or die($db->ErrorMsg());
            if($result->RecordCount() == 0) {
            ...
            if($user_info['account_type'] == 'senior_admin' && isset($_REQUEST['group_email_address']) && $_REQUEST['group_email_address'] != '') {
                $group_id = get_user_id(strtolower($_REQUEST['group_email_address']));
                $q = "select 'email' from accounts where user_id='$group_id' and account_type='group_admin'";
                $result = $db->Execute($q) or die($db->ErrorMsg());
                if($result->RecordCount() == 0) {
            ...
            if($change_group == 1)
                $log_entry .= " and changed group_owner to ".get_groupowner_name($group_id);
