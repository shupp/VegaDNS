...
$class = 'OS_'.$os;
 ...
 if(isset($_POST['RAM']))
 {
	...
	//var_dump($total);
    $use = ceil($total['total']) - ceil($total['free']);    
     $percent = ($use / ceil($total['total']));
     $percent = substr(($percent * 100),0,5);
	$response = (substr($total['total']/(1024*1024*1024),0,5)).'/'.(substr($total['free']/(1024*1024*1024),0,5)).'/'.$percent;
	echo $response;
 }
